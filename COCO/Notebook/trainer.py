...
from box_utils import yolov2_bbox_iou, xywh2xyxy
from anchor_utils import parse_anchors
import numpy as np


class Yolov2Trainer(torch.nn.Module):

    def __init__(self):
        super(Yolov2Trainer, self).__init__()
        self.yolov2 = Backbone().to(opt.device)
    ...
    def reorg_layer(self, preds):
        ...
        # ratio format is [h,w]
        ratio = torch.tensor([self.img_size[0], self.img_size[1]]) / torch.tensor(grid_size, dtype=torch.float32)
        ...
        box_xy, box_wh, confs_logit, classes_logit = torch.split(preds, [2, 2, 1, opt.class_num], dim=-1)
    ...
    def compute_loss_v3(self, preds, ground_truth):
        ...
        if opt.use_smooth_labels:
            true_classes = self.smooth_labels(ground_truth[..., 5:], opt.class_num)
        else:
    ...
    def compute_loss_v2(self, preds, ground_truth):
        """
        全部使用MSE
        :param preds:
        ...
        conf_mask, coord_mask, class_mask, target_confs, target_txty, target_twth, target_classes = self.build_targets(pred_bbox, ground_truth)
        ...
        # [batch_size, 13, 13, 5] / box with smaller area has higer weight
        # twth_loss_scale = 2. - (ground_truth[..., 2] / self.img_size[1]) * (ground_truth[..., 3] / self.img_size[0])
        ...
        conf_loss = mse(pred_confs * conf_mask, target_confs * conf_mask) / opt.batch_size

        # [batch_szie, 13, 13, 5] -> [batch_size, 13, 13, 5, 1]
        class_mask = class_mask.unsqueeze(dim=-1)
        pred_classes = torch.softmax(pred_classes, dim=-1)
        class_loss = mse(pred_classes * class_mask, target_classes * class_mask) / opt.batch_size
    ...
    def compute_loss(self, preds, ground_truth):
        ...
        # target_twth: [batch_size, 13, 13, 5, 2]
        # target_calsses: [batch_size, 13, 13, 5, 20]
        conf_mask, coord_mask, class_mask, target_confs, target_txty, target_twth, target_classes = self.build_targets(pred_bbox, ground_truth, iou_pred_gt)
        # conf_mask = torch.sqrt(conf_mask)
        ...
        pred_classes = pred_classes[class_mask].reshape(-1, opt.class_num)  # [N, 20]
    ...
    def build_targets(self, pred_bbox, ground_truth, iou_pred_gt):
        """
        :param pred_bbox:
            [batch_size, 13, 13, 5, 4]
        :param ground_truth:
        ...
        # [batch_size, 13, 13, 5]
        conf_mask = torch.ones(opt.batch_size, grid_size[0], grid_size[1], opt.B, dtype=torch.float32, requires_grad=False, device=opt.device) * opt.noobj_scale
        ...
        for b in range(ground_truth.size(0)):
            ...
            conf_mask[b][obj_mask[b]] = opt.obj_scale
    ...
    def init_optimizer(self):
        params = [{'params': self.yolov2.pth_layer.parameters(), 'lr': opt.pth_lr},
                  {'params': self.yolov2.yolo_layer.parameters(), 'lr': opt.yolo_lr}]

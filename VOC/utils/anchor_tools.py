...
def cluster_anchors(voc_path, opt):
    ...
    with open(opt.anchors_path, 'a') as f:
        for wh in center_coordinate[:]:
            f.write(str(wh[0] * opt.img_size) + ',' + str(wh[1] * opt.img_size) + '\n')
    return center_coordinate, class_list


def parse_anchors(opt):
    if not os.path.exists(opt.anchors_path):
        cluster_anchors(opt.anchors_path)
    anchors = np.loadtxt(fname=opt.anchors_path, delimiter=',', usecols=[0, 1], skiprows=0, dtype=np.float32).reshape([-1, 2])

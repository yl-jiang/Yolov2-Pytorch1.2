...
def letterbox_resize(img, bbox, dst_size):
    """
    :param img:
    ...
    letterbox_img = np.full(shape=[dst_size[0], dst_size[1], 3], fill_value=128)
    org_img_shape = [img.shape[0], img.shape[1]]
    ratio = np.min([dst_size[0] / org_img_shape[0], dst_size[1] / org_img_shape[1]])
    ...
    dh = dst_size[0] - resized_shape[0]
    dw = dst_size[1] - resized_shape[1]
    letterbox_img[(dh//2):(dh//2+resized_shape[0]), (dw//2):(dw//2+resized_shape[1]), :] = resized_img
    resized_bbox = bbox * ratio
    resized_bbox[:, [1, 3]] += dh // 2
    resized_bbox[:, [0, 2]] += dw // 2
    letterbox_img = letterbox_img.astype(np.uint8)
    return letterbox_img, resized_bbox, ratio, dh, dw
...
VOC_BBOX_LABEL_DICT = dict(zip(VOC_BBOX_LABEL_NAMES, range(len(VOC_BBOX_LABEL_NAMES))))
VOC_BBOX_LABEL_DICT = dict(zip(VOC_BBOX_LABEL_NAMES, range(len(VOC_BBOX_LABEL_NAMES))))


def traverse_voc(ann_dir, save_path):
    assert Path(ann_dir).exists()
    file_obj_dicts = defaultdict(dict)

    for ann_path in tqdm(ann_dir.iterdir()):
        soup = BeautifulSoup(open(ann_path), 'lxml')
        ann_name = soup.find('filename').string
        soup_size = soup.find('size')
        img_h, img_w = int(soup_size.find('height').string), int(soup_size.find('width').string)
        objs = soup.find_all('object')

        file_obj_dicts[ann_name] = defaultdict(list)
        file_obj_dicts[ann_name]['height'] = img_h
        file_obj_dicts[ann_name]['width'] = img_w
        for obj in objs:
            obj_name = obj.find('name').string
            obj_bbox = obj.find('bndbox')

            obj_ymax = int(float(obj_bbox.find('ymax').string))
            obj_xmax = int(float(obj_bbox.find('xmax').string))
            obj_ymin = int(float(obj_bbox.find('ymin').string))
            obj_xmin = int(float(obj_bbox.find('xmin').string))

            file_obj_dicts[ann_name]['names'].append(obj_name)
            file_obj_dicts[ann_name]['boxes'].append([obj_xmin, obj_ymin, obj_xmax, obj_ymax])
    try:
        pickle.dump(file_obj_dicts, open(save_path, 'wb'))
